...
just like in the generation_only script. After ten generations, 3 best agents (the ones with the highest fitness score)
are selected and put into Scenario 2 to be trained for another 3000 episodes.
"""
...
def evaluate(render=False):
    ...
    ckpt_dir = '../checkpoints/generation_scenario_impl/'
    nets = []
    for i in [0, 1, 2]:
        matches = glob.glob(os.path.join(ckpt_dir, f"focal_agent_{i}_final.pt"))
    ...
    print(f"\n{'='*70}")
    print(f"generation_scenario_3000 | {SCENARIO_NAME} | {N_EPISODES} episodes")
    print(f"{'='*70}")
    ...
    with open("../results/results_generation_scenario_3000.csv", "w", newline="") as f:
        wc = csv.writer(f)
    ...
    print("Saved results_generation_scenario_3000.csv")
