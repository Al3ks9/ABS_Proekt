...
CLIP_RATIO = 0.2
VF_COEF = 0.5
ENT_COEF_START = 0.3    # entropy coef starts at 0.3 because that's the final value in training with generation implementation
ENT_COEF_END = 0.15     # ends with 0.15. Agents should have already converged to a policy
...
SAVE_EVERY = 100
SAVE_PATH = "../checkpoints/generation_scenario_impl/"
...
def train():
    ...
    nets = [AgentNetwork(N_AGENTS_TRAIN, ACT_DIM).to(device) for _ in range(N_FOCAL)]

    # Load all agents from existing checkpoints
    ckpt_dir = "../checkpoints/generation_impl/"
    for idx, agent_idx in enumerate([1, 2, 6]):  # The best 3 from the fitness function
        matches = glob.glob(os.path.join(ckpt_dir, f"agent_{agent_idx}_fitness*.pt"))
        assert len(matches) == 1, f"Expected 1 checkpoint, found {len(matches)}"
        nets[idx].load_state_dict(torch.load(matches[0], map_location=device))
        print(f"  Loaded gen9 agent_{agent_idx} → focal agent {idx}")
    ...
    lastact_buf = np.zeros((STEPS_EP, N_FOCAL, N_AGENTS_TRAIN), dtype=np.int64)
    ...
    for ep in tqdm(range(EPISODES)):
        ...
        last_actions = torch.zeros(N_AGENTS_TRAIN, dtype=torch.int64, device=device)
        ...
        for t in range(STEPS_EP):
            ...
            for i in range(N_FOCAL):
                ...
                lastact_buf[t, i] = last_actions.cpu().numpy()
                ep_ret[i] += float(env_rewards[i])
                shaped_rew[i] += float(shaped[i])

            last_actions[:N_FOCAL] = acts_t.detach()
            ...
            for _ in range(TRAIN_EPOCHS):
                ...
                for start in range(0, t_end, MINIBATCH):
                    ...
                    if torch.isnan(logits_mb).any():
                        print(f"  WARNING: NaN logits for agent {i} at ep {ep}, skipping minibatch")
                        continue
