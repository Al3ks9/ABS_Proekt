"""
This is just a script to continue training the agents that were trained for 3000 episodes in Scenario 2.
Entropy coefficient starts at 0.3, because that's the last value after first 3000 episodes training.
"""
...
CLIP_RATIO = 0.2
VF_COEF = 0.5
ENT_COEF_START = 0.3    # starts at 0.3 because first 3000 episodes end with 0.3
ENT_COEF_END = 0.15
...
SAVE_EVERY = 100
SAVE_PATH = "../checkpoints/scenario2_2_impl/"
...
# Comparison logging
COMPARE_WINDOW = 50   # rolling window for focal vs background return comparison
...
def train():
    ...
    print(f"Scenario '{SCENARIO_NAME}': {actual_focal} focal agents", flush=True)
    assert actual_focal == N_FOCAL, (
        f"Expected {N_FOCAL} focal agents, got {actual_focal}. "
        f"Check SCENARIO_NAME or update N_FOCAL."
    )

    ANCHOR_IDX = 0      # agent 0 is the good one — fine-tune only
    ANCHOR_LR = 1e-4    # lower LR for anchor — stabilize, don't drift
    COPY_LR = 1e-3      # normal LR for copies — they need to learn
    ...
    # Load all agents from existing checkpoints
    checkpoint_path = '../checkpoints/scenario2_impl/'
    for i, net in enumerate(nets):
        ckpt = os.path.join(checkpoint_path, f"focal_agent_{i}_final.pt")
        if os.path.exists(ckpt):
            net.load_state_dict(torch.load(ckpt, map_location=device))
            print(f"  Loaded focal_agent_{i} from {checkpoint_path}", flush=True)
        else:
            print(f"  WARNING: no checkpoint at {ckpt}, starting from scratch", flush=True)

    # Copy anchor weights onto the other agents, then add layer-wise noise
    def add_noise(net, cnn_scale, policy_scale):
        with torch.no_grad():
            for name, param in net.named_parameters():
                scale = cnn_scale if name.startswith('cnn') else policy_scale
                param.data += torch.randn_like(param.data) * scale

    for i in range(N_FOCAL):
        if i != ANCHOR_IDX:
            nets[i].load_state_dict(nets[ANCHOR_IDX].state_dict())
            add_noise(nets[i], CNN_NOISE, POLICY_NOISE)
            print(f"  agent_{i} initialized from agent_{ANCHOR_IDX} + noise (cnn={CNN_NOISE}, policy={POLICY_NOISE})", flush=True)

    # Per-agent optimizers — anchor gets lower LR
    optimizers = [
        torch.optim.Adam(net.parameters(), lr=(ANCHOR_LR if i == ANCHOR_IDX else COPY_LR))
        for i, net in enumerate(nets)
    ]
    scalers = [GradScaler("cuda") for _ in range(N_FOCAL)]
    print(f"  Anchor agent {ANCHOR_IDX}: LR={ANCHOR_LR}  |  Copies: LR={COPY_LR}", flush=True)
    ...
    for ep in tqdm(range(EPISODES)):
        ...
        for t in range(STEPS_EP):
            ...
            with torch.no_grad(), torch.amp.autocast("cuda"):
                all_cnn = [nets[i].encode(obs_tensors) for i in range(N_FOCAL)]
                cnn_feats = torch.stack([all_cnn[i][i] for i in range(N_FOCAL)])
        ...
        for i in range(N_FOCAL):
            ...
            opt_i = optimizers[i]
            scaler_i = scalers[i]
            ent_coef = ent_coef_anchor if i == ANCHOR_IDX else ent_coef_copy
            net_i = nets[i]
            opt_i = optimizers[i]
            scaler_i = scalers[i]

            obs_gpu = torch.from_numpy(obs_buf[:t_end, i]).to(device, non_blocking=True).float().div_(255.0).permute(0, 3, 1, 2).contiguous()
            act_gpu = torch.from_numpy(act_buf[:t_end, i]).to(device, non_blocking=True)
            logp_old_gpu = torch.from_numpy(logp_buf[:t_end, i]).to(device, non_blocking=True)
            adv_gpu = torch.from_numpy(adv_buf[:t_end, i]).to(device, non_blocking=True)
            ret_gpu = torch.from_numpy(ret_buf[:t_end, i]).to(device, non_blocking=True)
            lastact_gpu = torch.from_numpy(lastact_buf[:t_end, i]).to(device, non_blocking=True)

            idxs = torch.randperm(t_end, device=device)
            net_i.train()

            for _ in range(TRAIN_EPOCHS):
                idxs = idxs[torch.randperm(t_end, device=device)]
                for start in range(0, t_end, MINIBATCH):
                    mb = idxs[start : start + MINIBATCH]

                    obs_mb = obs_gpu[mb]
                    act_mb = act_gpu[mb]
                    logp_old_mb = logp_old_gpu[mb]
                    adv_mb = adv_gpu[mb]
                    ret_mb = ret_gpu[mb]
                    lastact_mb = lastact_gpu[mb]
                    ...
                    logits_mb = logits_mb.float()
                    if torch.isnan(logits_mb).any():
                        print(f"  WARNING: NaN logits for agent {i} at ep {ep}, skipping minibatch", flush=True)
                        continue
        ...
        # Store first 50 episodes as "baseline" before learning kicks in
        ...
        print(
            ...
            f"steps {t_end:4d}  "
            f"[{per_agent_str}]  "
            f"mean={ep_ret.mean():.2f}  "
            f"shaped={shaped_rew.mean():.2f}  "
            f"rolling{COMPARE_WINDOW}={rolling_mean:.2f}  "
            f"vs_base={vs_baseline:+.2f}  "
            f"ent_a={ent_coef_anchor:.2f}/c={ent_coef_copy:.2f}  "
        ...
        if (ep + 1) % SAVE_EVERY == 0:
            ...
            print(f"Saved checkpoints at episode {ep + 1}", flush=True)
